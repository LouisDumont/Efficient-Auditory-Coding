...
def update_kernels(kernels_dic, decomposition, residual, learning_rate=1, verbose=0):
    for (kernel_idx, coeff, position) in decomposition:
        if verbose==2: print('Updating kernel {} with coeff {} from postiion {}'.format(kernel_idx, coeff, position))
...
def train(kernels_dic, sounds, n_epochs, learning_rate=0.1, init_threshold=1, verbose=0):
    ...
    if verbose==2: plot_sound(kernels_dic[0], title='kernel 0')
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(n_sounds):
            ...
            if verbose==2: plot_sound(sound, title='original sound')
            decomp, residual = mp_decomposition(sound, kernels_dic, max(threshold, 0.1), verbose=verbose)
            if verbose==2: plot_sounds([sound, residual], ['original sound', 'residual'])
            if verbose==2:
                print('decomp:',decomp)
        ...
        if verbose>0: print('Mean residual norm at epoch {}: {}'.format(epoch, epoch_score))
